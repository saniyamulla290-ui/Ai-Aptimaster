...
echo.
set /p mongo_done="Have you completed MongoDB setup? (y/n): "

if /i not "%mongo_done%"=="y" (
    echo Please complete MongoDB setup first, then run this script again.
    pause
    exit /b 0
)

REM Step 2: Backend Deployment
echo.
...
echo    - JWT_SECRET=generate_a_secure_random_string
echo    - CORS_ORIGIN=https://aptimaster-ai.vercel.app
echo 7. Click 'Create Web Service'
echo.
set /p backend_done="Have you deployed the backend? (y/n): "

if /i not "%backend_done%"=="y" (
    echo Please deploy the backend on Render first.
    pause
    exit /b 0
)

REM Step 3: Frontend Deployment
echo.
...
echo 7. Click 'Deploy'
echo.
set /p frontend_done="Have you deployed the frontend? (y/n): "

if /i not "%frontend_done%"=="y" (
